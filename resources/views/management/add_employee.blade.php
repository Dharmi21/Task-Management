...
@section('body')
        ...
        <tbody>
            @foreach ($employee_id as $key => $table_details)
                <tr>
                    ...
                    <td>
                        <p class="fw-normal mb-1">{{ $table_details->manager->name }}</p>
                    ...
                    <td>
                        <p class="fw-normal mb-1">{{ $table_details->created_at->format('d-m-y') }}</p>
                    </td>
                    <td>
    ...
    {{-- Edit modal --}}
    <form method="POST" action="{{ route('employee_update') }}">
        @csrf
        @method('PUT')
        ...
        <div class="modal fade" id="editmodal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <x-input type="text" hidden name="employee_id" id="employee_id"></x-input>

                        <x-input placeholder="Enter Name Employee" type="text" name="up_name"
                            id="up_name"></x-input>
                        <x-input-error :mess="$errors->get('up_name')" />

                        <br>
                        <x-input placeholder="Enter Email Of Employee" type="text" name="up_email"
                            id="up_email"></x-input>
                        <x-input-error :mess="$errors->get('up_email')" />
                        <x-input id="password" name="password" type="password" value="12345" hidden></x-input>

                        <br>

                        <x-dropdown id='up_department' name='up_department'>
                            <option value="">--Select Department--</option>
                        ...
                        <x-input-error :mess="$errors->get('up_department')" />
                        <br>
                        <x-dropdown id="up_manager" name='up_manager'>
                            <option value="">--Select Reporting Manager--</option>
                        ...
                        <x-input-error :mess="$errors->get('up_manager')" />
                        <br>
                        <x-input placeholder="Add Job Title" id='up_job' name='up_job'></x-input>
                        <x-input-error :mess="$errors->get('up_job')" />
            ...
            $(document).on('click', '.editbtn', function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $('#up_name').val(response.employee_data.name);
                        $('#up_email').val(response.employee_data.email);

                        $('#up_department').val(response.employee_data.department);
                        $('#up_manager').val(response.employee_data.reporting_manager);
                        $('#up_job').val(response.employee_data.job_title);
